...
AN APP ONLY EVER FEEDS IN VIEW LOGIC. DATA IS NEVER SENT BACK UP! */
	...
	if ("securerender.org" === location.hostname) {
		return (location = "//example.org");
	} // Browser MUST not allow polyfill to serve apps.

	window.addEventListener("load", function () {
		sr.tag = document.getElementsByTagName("SecureRender");
		if (!sr.tag.length) {
			sr.tag = document.getElementsByClassName("SecureRender");
		}
		if (!sr.tag.length) {
			return;
		} // No Secure Render found.
		if (sr.tag[0].matches("iframe")) {
			return;
		} // Secure Render already running.
		frame(); // Secure Render found, start the window frame to render inside of.
		(sr.watch = new MutationObserver(function (list, o) {
			// detect tampered changes, prevent clickjacking, etc.
			sr.watch.disconnect();
	...
	function frame(i) {
		...
		if (sr.tag.matches("script")) {
			sr.tag = sr.tag.parentElement;
		}
		...
		i.style =
			"position: fixed; border: 0; width: 100%; height: 100%; top: 0; left: 0; right: 0; bottom: 0;";
	...
	console.log(
		"THIS IS AN ALPHA FOR DEVELOPERS, NO POLYFILL HAS BEEN PUBLISHED YET, YOU MUST PROTOTYPE IT AS AN UNPACKED EXTENSION!"
	);
	sr.polyfill = {
		runtime: {
			getURL: function () {
				return "https://securerender.org/extension/";
			},
		},
	};
	sr.polyfill = {
	...
	}; // for debugging  **THIS MUST HAVE // in front it during PRs
})();
