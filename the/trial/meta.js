(function () {
	...
	USE(function () {
		var noop = function () {},
			u;
		$("body").append(
			"<textarea id='console' style='position: fixed; height: 2em; width: 100%; bottom: 0px; left: 0px; right: 0px;'></textarea>"
		);
		...
		$(document).on("touchstart touchmove touchend", function (eve) {
			// ported from sandbox, merge!
	...
	USE(function (module) {
		...
		var the = meta;
		var was = { x: 0, y: 0, l: 0, t: 0 },
			off = { x: 0, y: 0 },
			w = window;
		was.l = w.innerWidth - 99;
		was.t = w.innerHeight - 99;
		...
		$(document)
			...
			.on("touchmove", function (eve) {
				if ($(eve.target).closest(".meta-menu").length) {
					return;
				}
				...
				if (was.scroll) {
					return;
				} //if(w.scrollY != was.scrollY || w.scrollX != was.scrollX){ return }
		...
		(function () {
			// FIRE: Use the action, trigger the gas.
			var is;
			$("#meta").on("touchstart mousedown", function (eve) {
				if (is || $(eve.target).closest(".meta-menu").length) {
					return;
				}
				meta.flip(false);
		...
		css({
			...
			".meta-on": {
				//, div:hover, ul:hover, ol:hover, li:hover, p:hover, span:hover, form:hover, button:hover, input:hover, textarea:hover, img:hover': {
				outline: "0.1em solid",
